/* ============================================
   Gerald's Private Web v2 — modules/sosial.js
   Load & render sosial, komunitas, karya
   dari package.json
   ============================================ */
...
(function (G) {
  ...
  var ICONS = {
    ...
    ngl: '<svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 3c1.66 0 3 1.34 3 3s-1.34 3-3 3-3-1.34-3-3 1.34-3 3-3zm0 14.2c-2.5 0-4.71-1.28-6-3.22.03-1.99 4-3.08 6-3.08 1.99 0 5.97 1.09 6 3.08-1.29 1.94-3.5 3.22-6 3.22z"/></svg>'
  };
  ...
  G.Sosial = {
    ...
    renderKomunitas: function (cfg) {
      var grid = document.getElementById('komunitasGrid');
      if (!grid) return;
      ...
      if (!enabled.length) {
        grid.innerHTML = '<p class="sosial-empty">Belum ada komunitas.</p>';
        return;
      }
      ...
      for (var i = 0; i < enabled.length; i++) {
        var c = enabled[i];
        html += '<a class="komunitas-card glass-card" href="' + (c.url || '#') + '" target="_blank" rel="noopener">';
        html += '<span class="komunitas-name">' + c.name + '</span>';
        if (c.description) html += '<span class="komunitas-desc">' + c.description + '</span>';
        html += '<span class="komunitas-arrow">→</span>';
        html += '</a>';
      }
    ...
    renderKarya: function (cfg) {
      ...
      if (!enabled.length) {
        grid.innerHTML = '<p class="sosial-empty">Belum ada karya.</p>';
        return;
      }
      ...
      for (var i = 0; i < enabled.length; i++) {
        var w = enabled[i];
        html += '<a class="karya-card glass-card" href="' + (w.url || '#') + '" target="_blank" rel="noopener">';
        html += '<span class="karya-title">' + w.title + '</span>';
        if (w.description) html += '<span class="karya-desc">' + w.description + '</span>';
        html += '</a>';
      }
